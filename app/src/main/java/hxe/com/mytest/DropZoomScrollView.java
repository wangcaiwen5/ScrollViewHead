...
public class DropZoomScrollView extends ScrollView implements View.OnTouchListener{
    private float mFristePosition=0;
    //是否正在滑动
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (!mScrolling) {
                    if (getScrollY() == 0) {
                        mFristePosition = event.getY();// 滚动到顶部时记录位置，否则正常返回
                    } else {
                        break;
                    }
                }
                int distance = (int) ((event.getY() - mFristePosition) * 0.6); // 滚动距离乘以一个系数
